...
#-------------------------------------------------------------------------------
  ...
  def dispatch( self ):
    #base.cam.node().getDisplayRegion( 0 ).setActive( False ) #last
    #base.cam.node().getDisplayRegion( 0 ).getWindow().setClearColorActive( False )
    #base.cam.node().getDisplayRegion( 0 ).getWindow().setClearDepthActive( False )
    ...
    while len( self.times ) > 0 and self.times[0] <= currentTime:
      ...
      for task in taskList:
        period = task["period"]
        if not self.skipFrames or not processed.has_key( task["name"] ) or period == 0:
    ...
    for task in self.renderTasks:
      ...
      if not result:
        self.removeTask( task["name"] )
    #base.cam.node().getDisplayRegion( 0 ).setActive( True ) #last
    #base.cam.node().getDisplayRegion( 0 ).getWindow().setClearColorActive( True )
    #base.cam.node().getDisplayRegion( 0 ).getWindow().setClearDepthActive( True )
