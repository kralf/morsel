...
class KeyboardController( object ):
  ...
  def __init__( self, platform, axis1Time = 1, axis2Time = 1 ):
    ...
    self.increments = [
      platform.limits[0][1] - platform.limits[0][0] / axis1Time,
      platform.limits[1][1] - platform.limits[1][0] / axis2Time
    ]
    self.lastTime = 0
  ...
  def step( self, time ):
    ...
    if self.keyMap["forward"] != 0:
      ...
      v[a] = min( v[a], self.platform.limits[a][1] )
    elif self.keyMap["back"] != 0:
      v[a] -= self.increments[a] * delta
      v[a] = max( v[a], self.platform.limits[0][0] )
    else:
      if v[a] != 0:
    ...
    if self.keyMap["left"] != 0:
      ...
      v[a] -= self.increments[a] * delta
      v[a] = max( v[a], self.platform.limits[a][0] )
    else:
      if v[a] != 0:
        v[a] = v[a] - self.increments[a] * delta * v[a] / abs( v[a] )
        if abs( v[a] ) <= abs( self.increments[a] * delta ):
